...
function atualizarFaturamento() {
    // Receita realizada do período = SOMENTE agendamentos avulsos concluídos.
    // Pacotes são acompanhados separadamente no KPI "Pacotes ativos" e não entram
    // novamente em Receita realizada, Atendimentos ou Ticket médio.
    const avulsos = obterAgendamentosFiltradosFaturamento();

    const quantidade = avulsos.length;
    const valorTotal = avulsos.reduce((acc, item) => acc + Number(item.valorTotal || 0), 0);
    ...
    const contexto = calcularContextoFinanceiro(avulsos, valorTotal, ticketMedio);
    renderizarInteligenciaFinanceira(avulsos, contexto);
    renderizarGraficos(avulsos);
...
function calcularContextoFinanceiro(dados, valorTotal, ticketMedio) {
    ...
    const receitaMes = avulsosMes.reduce((a,i)=>a+Number(i.valorTotal||0),0);
...
function renderizarComparacaoFinanceira(dados, c) {
    ...
    const anterior = avulsosAnterior;
